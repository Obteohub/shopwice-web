...
interface TelevisionsProductsProps {
    products: Product[];
}

const TelevisionsProducts = ({ products }: TelevisionsProductsProps) => {
        ...
        <div className="bg-white py-1 border-t border-gray-100">
            <div className="w-full px-2 sm:px-6 relative group">
                <div className="flex items-center justify-between mb-2 pb-2">
                    <h2 className="text-xl text-gray-900 tracking-tight">Televisions</h2>
                    <Link href="/product-category/televisions" className="text-blue-600 text-md font-bold hover:underline">
                        View All
                    </Link>
                </div>
                ...
                <div className="md:hidden flex overflow-x-auto snap-x snap-mandatory gap-1 pb-4 no-scrollbar">
                    {products.map((product) => (
                        <div key={`mob-tvs-${product.databaseId}`} className="snap-center shrink-0 w-[50vw]">
                            <ProductCard
                ...
                <div className="hidden md:block overflow-hidden relative min-h-[400px]">
                    ...
                    >
                        {products.map((product) => (
                            <div
                                key={`desk-tvs-${product.databaseId}`}
                                className="px-2"
                                style={{
